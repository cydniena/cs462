...
const RoomStats = ({ bookings = [], occupancyData = [], roomName = "",  timeRange = "", selectedDate = "", selectedHour=""}) => {
  ...
  const filteredData = occupancyData.filter(item => {
    ...
    if (timeRange === "hour") {
      ...
      return selectedWeek === itemWeek && 
             new Date(itemDate).getFullYear() === new Date(selectedDate).getFullYear() &&
             itemHour >= OPERATING_HOURS.start && 
             itemHour < OPERATING_HOURS.end;
    }
    return true;
   ...
   const filteredBookings = bookings.filter(booking => {
    ...
    const bookingDate = new Date(booking.BookingStartTime).toISOString().split('T')[0];
    const bookingHour = new Date(booking.BookingStartTime).getHours();
    
    if (timeRange === "hour") {
      return bookingDate === selectedDate && bookingHour === parseInt(selectedHour);
    } else if (timeRange === "day") {
      return bookingDate === selectedDate;
    } else if (timeRange === "week") {
      const selectedWeek = getWeekNumber(new Date(selectedDate));
      const bookingWeek = getWeekNumber(new Date(bookingDate));
      return selectedWeek === bookingWeek && 
             new Date(bookingDate).getFullYear() === new Date(selectedDate).getFullYear();
    }
    return true;
  });
  ...
  const calculateUtilization = () => {
   ...
   const calculateBookedHours = () => {
    if (timeRange === "hour") {
      // For hourly view, check if there's any booking for that hour
      const isBooked = filteredBookings.some(booking => {
        const bookingHour = new Date(booking.BookingStartTime).getHours();
        return bookingHour === parseInt(selectedHour);
      });
      return {
        booked: isBooked ? 1 : 0,
        total: 1
      ...
      const hourSet = new Set(); // To track unique hours with bookings
      
      filteredBookings.forEach(booking => {
        const start = new Date(booking.BookingStartTime);
        const end = new Date(booking.BookingEndTime);
        
        // Calculate hours for this booking (rounding up to count partial hours)
        const hours = Math.ceil((end - start) / (1000 * 60 * 60));
        
        // Add each hour of the booking to our set
      ...
      filteredBookings.forEach(booking => {
        ...
        // Add each hour slot to our set
        for (let i = 0; i < hours; i++) {
          const time = new Date(start.getTime() + i * 60 * 60 * 1000);
          const hour = time.getHours();
          if (hour >= OPERATING_HOURS.start && hour < OPERATING_HOURS.end) {
            // Create unique key combining date and hour
            const dateStr = time.toISOString().split('T')[0];
            hourSet.add(`${dateStr}-${hour}`);
          }
        }
      });
      ...
      return (
        <div className="grid grid-cols-2 gap-4 mt-5 text-lg">
          <div className="p-4 bg-white shadow-md rounded-lg border">
            ...
            <p className="text-xl font-bold text-blue-600">{utilized}hr/{total}hr</p>
          </div>
    
          <div className="p-4 bg-white shadow-md rounded-lg border">
            <p className="font-semibold text-gray-700">Not Utilized Hours</p>
            <p className="text-xl font-bold text-red-600">{total - utilized}hr/{total}hr</p>
          </div>
    
          <div className="p-4 bg-white shadow-md rounded-lg border">
            <p className="font-semibold text-gray-700">Booked Hours</p>
            <p className="text-xl font-bold text-green-600">{booked}hr/{total}hr</p>
          </div>
    
          <div className="p-4 bg-white shadow-md rounded-lg border">
            <p className="font-semibold text-gray-700">Unbooked Hours</p>
            <p className="text-xl font-bold text-orange-600">{total - booked}hr/{total}hr</p>
          </div>
        </div>
      );
    };  
